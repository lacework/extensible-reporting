...
def events_raw(events, severities=["Critical", "High"], excluded_event_types=["CloudTrailDefaultAlert", "CloudActivityLogIngestionFailed", "NewViolations", "ComplianceChanged"], limit=False):
    ...
    # format time
    df['START_TIME'] = df['START_TIME'].apply(lambda x: datetime.strptime(x,"%Y-%m-%dT%H:%M:%SZ").strftime("%B %d, %Y %I:%M%p"))
    
    # delete extra columns
    df = df[['EVENT_ID', 'SEVERITY', 'START_TIME', 'EVENT_TYPE_LONG']]
    
    # rename columns
    df.rename(columns={'EVENT_ID': 'Event ID', 'SEVERITY': 'Severity', 'START_TIME': 'Event Time', 'EVENT_TYPE_LONG': 'Event Type'}, inplace=True)
